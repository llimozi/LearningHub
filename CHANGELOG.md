...
> 更新于 2026-09-03 · 语义纵深（v1.15）· LLM 周报洞察 + 跨概念语义关联（复用熔断基建）

---

## 【v1.15 · LLM 语义纵深（Phase B · 可选增强）】

> 把 LLM 从「单篇概念提炼」推进到「周报洞察 + 图谱语义结构」，全部**复用 analyzer 的 `_call_deepseek` + `_LLM_DISABLED` 熔断基建**（单一出口）。无 Key / 熔断时完整回退既有行为，功能不缺，零二次调用。

- **B1 周报智能摘要（services.weekly_report）** — 收敛重复建设：原自建 urllib DeepSeek 分支（services.py:598-616，自管 25s 超时/无 JSON 模式/无熔断）**整体移除**，改为调用 `analyzer.llm_weekly_insight`。聚合真实素材（概念摘要 + 复习趋势：近 7 天复习次数、掌握 <40 概念数）生成「四、本周洞察（AI）」≤150 字；无 Key/熔断/失败 → 完整回退纯模板。
- **B2 跨概念语义关联（graph.py + analyzer.py）** —
  - `analyzer.llm_concept_relations`：判定概念对关系类型（上位 / 下位 / 相关 / 前置），类型白名单校验、非法值兜底「相关」、概念名小写规范化（与 build_graph 缓存键一致）；
  - `graph.enrich_relations`：**带缓存**（写回 `knowledge.json` 顶层 `semantic_rels`）——已判定对绝不重复调 API；**批量 ≤10 对/批**；无 Key/熔断/失败静默停批保留已缓存；
  - `/api/graph` 响应**追加 `semantic_rels`**（空 dict 兼容，前端旧代码忽略未知字段；可据此把图谱边从「共现」升级为语义描线）。
  - 小修记录：`enrich_relations` 返回字段 `"new": len(rels)` 名不副实（实为缓存**总数**非新增数），改为 **`"cached_total"`**。
- **B3 语义自适应调度（难度注入）** — LLM 估难度仅作**冷启动增强**（建档可覆盖默认 D）；无 Key 时 D 完全由遗忘表现驱动（Phase A2 S/D 闭环），两条路径均有测试。
- **llm_semantic_test.py（新增 13 例）** — 全部 mock 注入不触真实 API：无 Key 零调用 / 成功追加 / 失败熔断+零二次调用 / 类型校验与兜底 / 缓存去重（二次零调用）/ 批量限制 / 图 API 带出 / 难度注入与默认。`_reset_fuse()` 每例复位熔断态。
- **终验**：业务测试 **45 套 / 505 例 / 0 失败**（anaconda3 基线运行时）。

---

## 【v1.14 · 学习科学调度算法（FSRS 双参数 + 真实校准 + Benchmark）】

> Phase A（核心）：把 forgetting.py 从「SM-2 简化（单参数 ease + 固定档位表）」升级为 FSRS 风格双参数调度（stability + difficulty），并用真实复习记录拟合遗忘曲线，产出可量化 Benchmark。**零第三方依赖、纯本地、向后兼容。**

- **FSRS 式调度内核（forgetting.py）** — 引入「稳定性 S + 难度 D」双参数：`knowledge.json` 版本 **v1 → v2**（记录新增 `stability`/`difficulty`）。旧 v1 数据（无这两个字段）**读取与行为完全回退 v1.1 逻辑**（固定档位表 + 指数保持率），绝不崩、绝不变行为；新记录（sync 建档）直接携带初始 S/D。
- **复习结果驱动更新（mark_reviewed）** — 每次复习的 quality 同时驱动 S 与 D：答对（q≥3）`S ×= growth(q)`（3→1.6 / 4→2.0 / 5→2.5）且 `D ×= 0.95`；答错（q<3）`S ×= 0.5` 且 `D = min(1.0, D+0.2)`；钳制 `S∈[1,3650]`、`D∈[0.1,0.95]`。既有行为（review_count+1、last_review_ts、review_log 追加、snooze_until 解除）一律不变；v1 `ease_factor` 双轨保留（不中断任何外部读取）。
- **真实遗忘曲线校准（calibrate_decay + 周报钩子）** — 用 `review_log` 真实复习记录拟合 DECAY：网格搜索 `[0.05, 0.50]` 步进 0.01，最小化「预测保持率 vs 实际复习表现」的 log-loss（S 取该概念当前 stability）。**样本 <30 条静默返回默认**（真实库当前仅 2 条，走此分支）；异常静默降级。挂载 `reportio.ensure_reports`（每周低频），结果写回 `knowledge.json` 顶层 `decay` 字段。
- **可量化 Benchmark（scheduler_benchmark.py，新增独立脚本）** — 100 概念独立时间线 × 365 天，确定性种子 `SEED=20260903` 可复算；对比「旧 SM-2 档位 + ease」与「新 FSRS S/D」。产物落盘 `tests_output/scheduler_benchmark.md`。

### Benchmark 结果（诚实呈现，数字可复算）

| 指标 | 旧 v1.1（SM-2 档位 + ease） | 新 v2.0（FSRS S/D） | 读数 |
|---|---|---|---|
| 总复习次数（365 天 × 100 概念） | 2160 | **1158（−46.4%）** | ↓ 少=省力 |
| 到期时真实保持率 | 79.3% | 78.9% | ≈ 持平 |
| 间隔中位数 | 16 天 | 18 天 | ↑ |
| 间隔 P90 | 30 天 | **71 天** | ↑ 稳定概念被稀疏化 |
| 全年平均真实保持率（健康度） | **85.7%** | 75.3% | ↑ 高=更牢 |

- **新调度占优**：复习负担 **−46.4%**（S 随成功复习指数增长，稳定概念更少被打扰）。
- **新调度不占优（如实呈现）**：全年健康度 **75.3% < 85.7%**。根因：调度 DECAY 用默认 **0.09**（假设衰减慢），而模拟真实衰减为 **0.20**，导致间隔拉得比实际记忆能支撑的更长 → 到期时真实保持率降到 ~79%（目标 90%）。**这正是 A3 真实校准的价值所在**：真实库积累 ≥30 条复习记录并完成校准后，DECAY 会自适应收敛，健康度预期改善；**未为好看而调参或挑选指标**。
