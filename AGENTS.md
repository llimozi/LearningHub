...
- 第一轮治理成果（基线，不得回退）：
  - P1 技术债务全部清零
  - 业务测试 **45 套 / 505 例 / 0 失败**（入口：`run_tests.py`；含 icon_contract_test 8 例匹配型图标契约、analyzer_llm_test 7 例、forgetting_test 27 例、llm_semantic_test 13 例）
...
- 任何涉及模板、CSS、交互的改动后必须依次执行：
  1. 业务测试：`python run_tests.py`（45 套 / 505 例，0 失败为门禁；建议用带 tkinter 的完整 Python 发行版，见 README「环境差异说明」）
